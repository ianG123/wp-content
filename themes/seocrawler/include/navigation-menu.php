<?php
	...
	*/
	...
	add_filter('gdlr_core_custom_menu_widget_walker', 'seocrawler_gdlr_core_custom_menu_widget_walker');
	if( !function_exists('seocrawler_gdlr_core_custom_menu_widget_walker') ){
		function seocrawler_gdlr_core_custom_menu_widget_walker( $walker ){
			return new seocrawler_menu_walker();
		}
	}

	// custom menu
	if( !function_exists('seocrawler_get_custom_menu') ){
		function seocrawler_get_custom_menu( $settings = array() ){
			if( !empty($settings['type']) ){
				if( $settings['type'] == 'overlay' ){
					seocrawler_get_overlay_menu($settings);
				}else if( $settings['type'] == 'left' || $settings['type'] == 'right' ){
					$settings['slide'] = $settings['type'];
					seocrawler_get_mmenu($settings);
				}
			}
		}
	}
	...
	// menu icon
	if( !function_exists('seocrawler_get_mobile_menu_icon') ){
		function seocrawler_get_mobile_menu_icon( $settings = array() ){

			$settings = wp_parse_args($settings, array(
				'href' => '#',
				'button-type' => seocrawler_get_option('general', 'right-menu-style', 'hamburger-with-border'),
 				'button-class' => '',
				'icon-class' => 'icon_menu'
			));

			$button_class  = $settings['button-class'];
			$button_class .= ' seocrawler-mobile-button-' . $settings['button-type'];
	...
	// overlay menu
	if( !function_exists('seocrawler_get_overlay_menu') ){
		function seocrawler_get_overlay_menu( $settings = array() ){
			...
			echo '<div class="seocrawler-overlay-menu ' . esc_attr($settings['container-class']) . '" id="' . esc_attr($settings['id']) . '" >';
			
			$settings['button-class'] = 'seocrawler-overlay-menu-icon ' . $settings['button-class'];
			seocrawler_get_mobile_menu_icon($settings);

			echo '<div class="seocrawler-overlay-menu-content seocrawler-navigation-font" >';
			echo '<div class="seocrawler-overlay-menu-close" ></div>';

			echo '<div class="seocrawler-overlay-menu-row" >';
			echo '<div class="seocrawler-overlay-menu-cell" >';
			wp_nav_menu(array(
	...
	// mmenu
	if( !function_exists('seocrawler_get_mmenu') ){
		function seocrawler_get_mmenu( $settings = array() ){
			...
			$settings['button-class'] = 'seocrawler-mm-menu-button ' . $settings['button-class'];
			$settings['href'] = '#' .  $settings['id'];
			seocrawler_get_mobile_menu_icon($settings);

			echo '<div class="seocrawler-mm-menu-wrap seocrawler-navigation-font" id="' . esc_attr($settings['id']) . '" data-slide="' . esc_attr($settings['slide']) . '" >';
			wp_nav_menu(array(
				'theme_location'=>$settings['theme-location'], 
				'container'=> '', 
				'menu_class'=> 'm-menu'
	...
	if( class_exists('gdlr_core_edit_nav_menu') ){
		new gdlr_core_edit_nav_menu(array(
			'icon-class' => array(
				'title' => esc_html__('Icon Class', 'seocrawler'),
				'type' => 'text',
			),
			'enable-mega-menu' => array(
				'title' => esc_html__('Enable Mega Menu', 'seocrawler'),
				'type' => 'checkbox',
				'depth' => '0'
			),
			'mega-menu-background' => array(
				'title' => esc_html__('Mega Menu Background', 'seocrawler'),
				'type' => 'upload',
				'depth' => '0'
			),
			'mega-menu-background-position' => array(
				'title' => esc_html__('Mega Menu Background Position', 'seocrawler'),
				'type' => 'combobox',
				'options' => array(
					'center' => esc_html__('Center', 'seocrawler'),
					'top-left' => esc_html__('Top Left', 'seocrawler'),
					'top-center' => esc_html__('Top Center', 'seocrawler'),
					'top-right' => esc_html__('Top Right', 'seocrawler'),
					'center-left' => esc_html__('Center Left', 'seocrawler'),
					'center-right' => esc_html__('Center Right', 'seocrawler'),
					'bottom-left' => esc_html__('Bottom Left', 'seocrawler'),
					'bottom-center' => esc_html__('Bottom Center', 'seocrawler'),
					'bottom-right' => esc_html__('Bottom Right', 'seocrawler'),
				),
				'depth' => '0'
			),
			'mega-menu-background-repeat' => array(
				'title' => esc_html__('Mega Menu Background Repeat', 'seocrawler'),
				'type' => 'combobox',
				'options' => array(
					'cover' => esc_html__('Cover ( full width and height )', 'seocrawler'),
					'no-repeat' => esc_html__('No Repeat', 'seocrawler'),
					'repeat' => esc_html__('Repeat X & Y', 'seocrawler'),
					'repeat-x' => esc_html__('Repeat X', 'seocrawler'),
					'repeat-y' => esc_html__('Repeat Y', 'seocrawler'),
				),
				'default' => 'cover',
				'condition' => array( 'background-type' => 'image' )
			), 
			'mega-menu-width' => array(
				'title' => esc_html__('Mega Menu Width ( Fill value with % or px )', 'seocrawler'),
				'type' => 'text',
				'default' => '100%',
				'depth' => '0'
			),
			'hide-menu-title' => array(
				'title' => esc_html__('Hide Menu Title', 'seocrawler'),
				'type' => 'checkbox',
				'depth' => '1'
			),
			'mega-menu-section-size' => array(
				'title' => esc_html__('Section Size ( Only for mega menu )', 'seocrawler'),
				'type' => 'combobox',
				'options' => array( 
			...
			'mega-menu-section-content' => array(
				'title' => esc_html__('Section Content ( Only for mega menu )', 'seocrawler'),
				'type' => 'textarea',
				'depth' => '1'
			),

			'side-text' => array(
				'title' => esc_html__('Side Text (Custom Menu Widget)', 'seocrawler'),
				'type' => 'text',
				'depth' => '0'
			),
			'side-text-bg' => array(
				'title' => esc_html__('Side Text Background (Custom Menu Widget)', 'seocrawler'),
				'type' => 'colorpicker',
				'depth' => '0'
			),
		));
	...
	if( !class_exists('seocrawler_menu_walker') ){
		
		// from wp-includes/nav-menu-template.php file
		class seocrawler_menu_walker extends Walker_Nav_Menu{
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				if( $depth == 0 ){
					...
					if( ceil($this->top_level_items / 2) + 1 == $this->top_level_count ){
						$center_nav_item = apply_filters('seocrawler_center_menu_item', '');
						if( !empty($center_nav_item) ){
							$output .= '<li class="seocrawler-center-nav-menu-item" >' . $center_nav_item . '</li>';
						}
					}
				}
				...
				if( $depth == 0 ){
					if( $item->gdlr_core_nav_menu_custom['enable-mega-menu'] == 'disable' ){
						$class_names .= ' seocrawler-normal-menu';
					}else{
						$class_names .= ' seocrawler-mega-menu';
					}
				}else if( $depth == 1 ){
				...
				if( $depth == 1 && !empty($item->gdlr_core_nav_menu_custom['mega-menu-section-content']) ){
					$item_output .= '<div class="seocrawler-mega-menu-section-content">';
				...
				if( $depth == 0 && $item->gdlr_core_nav_menu_custom['enable-mega-menu'] == 'enable' ){
					...
					if( !empty($item->gdlr_core_nav_menu_custom['mega-menu-background']) ){
						...
						if( !empty($item->gdlr_core_nav_menu_custom['mega-menu-background-position']) ){
							...
							$mega_background .= ' background-position: ' . $background_pos . '; ';
						}
						if( !empty($item->gdlr_core_nav_menu_custom['mega-menu-background-repeat']) ){
							$mega_background .= ' background-repeat: ' . $item->gdlr_core_nav_menu_custom['mega-menu-background-repeat'] . '; ';
						}
					}
					...
					if( empty($item->gdlr_core_nav_menu_custom['mega-menu-width']) || trim($item->gdlr_core_nav_menu_custom['mega-menu-width']) == '100%' ){
						...
						$item_output .= '<div class="sf-mega" style="width: ' . esc_attr($item->gdlr_core_nav_menu_custom['mega-menu-width']) . '; ' . $mega_background . '">'; 
					}
